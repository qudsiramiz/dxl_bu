...
#    plot_duration=7
    ...
    # start = time.time()
    print(f"Code execution for ACE {plot_duration} days data started at at (UTC):" +
    ...
    ftp.login()
    ftp.cwd("/pub/lists/ace2/")
    files = ftp.nlst()
    swp_fl = "202112_ace_swepam_1h.txt"
    mag_fl = "202112_ace_mag_1h.txt"

    fl_path = f"/home/cephadrius/Desktop/git/dxl_bu/data/ace_ftp_data/"

    try:
        # Download the ACE data from website
        ftp.retrbinary("RETR " + swp_fl, open(f"{fl_path}{swp_fl}", 'wb' ).write)
        ftp.retrbinary("RETR " + mag_fl, open(f"{fl_path}{mag_fl}", 'wb' ).write)
    except Exception as e:
        print(e)
        pass

    # List of keys for the two files
    ace_key_list_mag=['year', 'month', 'date', 'utctime', 'julian_day', 'doy', 's', 'bx_gsm',
                      'by_gsm', 'bz_gsm', 'bt', 'lat_gsm', 'lon_gsm']
    ace_key_list_swp = ['year', 'month', 'date', 'utctime', 'julian_day', 'doy', 's', 'np', 'vp',
                        'Tp']

    # Read data from sweap and magnetometer in a dataframe
    df_ace_mag = pd.read_csv(f"{fl_path}{mag_fl}", sep=r'\s{1,}', skiprows=20, names=ace_key_list_mag,
                                 engine='python', dtype={"month":"string", "date":"string",
                                                         "utctime":"string"})
    df_ace_swp = pd.read_csv(f"{fl_path}{swp_fl}", sep=r'\s{1,}', skiprows=18, names=ace_key_list_swp,
                             engine='python', dtype={"month":"string", "date":"string",
                                                     "utctime":"string"})
    ...
    df_ace_mag.index = np.array([datetime.datetime.strptime(
        f'{df_ace_mag.year[i]}{df_ace_mag.month[i]}{df_ace_mag.date[i]}{df_ace_mag.utctime[i]}','%Y%m%d%H%M')
        for i in range(len(df_ace_mag.index))
        ])

    df_ace_swp.index = np.array([datetime.datetime.strptime(
        f'{df_ace_swp.year[i]}{df_ace_swp.month[i]}{df_ace_swp.date[i]}{df_ace_swp.utctime[i]}', '%Y%m%d%H%M')
    ...
    min_time = datetime.datetime.utcfromtimestamp(time.time()) - datetime.timedelta(days=plot_duration)
    ...
    fig_name = f"/home/cephadrius/Dropbox/DXL-Figure/sw_ace_parameters_{plot_duration}day.png"

...
def gif_maker(file_list, vid_name, mode="I", skip_rate=10, vid_type="mp4", duration=0.05, fps=25):
    ...
    if len(file_list) >= 1501:
        ...
        file_list = file_list[-3000::skip_rate]
    if vid_type == "gif":
...
def make_gifs(sc):
    ...
    s.enter(21600, 1, make_gifs, (sc,))

    print(f"Code execution started at (UTC):" +
          f"{datetime.datetime.utcfromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')}\n")
    file_list_dict = {}
    file_list_dict["file_list_2hr"] = np.sort(glob.glob("/media/cephadrius/endless/bu_research/dxl/figures/historical/dscovr/2hr/sw_dsco_*.png"))[-3000::1]

    skip_rate_list = [1]
    for i,key in enumerate(list(file_list_dict.keys())):
        vid_name = f"{gif_path}{key}_v2.{vid_type}"

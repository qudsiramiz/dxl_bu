...
def plot_figures_dsco_54days():
    ...
    print(
        f"Code execution for DSCOVR 54 days data started at at (UTC):"
    ...
    for mag_file, plas_file in zip(mag_file_list, plas_file_list):
        ...
        df_plas_list[count] = pd.DataFrame(
            {"np": plas_data_np, "vp": plas_data_vp}, index=plas_data_time_utc
        )
    ...
    df_dscovr["bm"] = np.sqrt(
        df_dscovr.bx_gsm**2 + df_dscovr.by_gsm**2 + df_dscovr.bz_gsm**2
    )
    ...
    # propagation time
    indx_min = min(
        range(len(df_dscovr.index)),
        key=lambda i: abs(
            df_dscovr.index[i] - df_dscovr.index[-1] + datetime.timedelta(minutes=35)
        ),
    )
    ...
    df_dscovr["lambda_phi"] = (
        -3.65e-2
        * (df_dscovr.vp * df_dscovr.bt * np.sin(df_dscovr.theta_c / 2.0) ** 4)
        ** (2 / 3)
        + 77.2
        + dipole_correction
    )
    df_dscovr["lambda_wav"] = (
        -2.27e-3 * (df_dscovr.vp * df_dscovr.bt * np.sin(df_dscovr.theta_c / 2.0) ** 4)
        + 78.5
        + dipole_correction
    )
    df_dscovr["lambda_vas"] = (
        -2.14e-4
        * df_dscovr.p_dyn ** (1 / 6)
        * df_dscovr.vp ** (4 / 3)
        * df_dscovr.bt
        * np.sin(df_dscovr.theta_c) ** 4
        + 78.3
        + dipole_correction
    )
    df_dscovr["lambda_ekl"] = (
        -1.90e-3 * df_dscovr.vp * df_dscovr.bt * np.sin(df_dscovr.theta_c / 2.0) ** 2
        + 78.9
        + dipole_correction
    )
    ...
    im1e = axs1.plot(df_dscovr.index.values, -df_dscovr.bm.values, "k-.", lw=lw, ms=ms)
    ...
    im2 = axs2.plot(
        df_dscovr.index.values, df_dscovr.np.values, "r-", lw=lw, ms=ms, label=r"$n_p$"
    )
    axs2.plot(
        df_dscovr_hc.index.values, df_dscovr_hc.np.values, color="r", lw=1, alpha=alpha
    )
    ...
    im3 = axs3.plot(
        df_dscovr.index.values, df_dscovr.vp.values, "b-", lw=lw, ms=ms, label=r"$V_p$"
    )
    axs3.plot(
        df_dscovr_hc.index.values, df_dscovr_hc.vp.values, color="b", lw=1, alpha=alpha
    )
    ...
    im4 = axs4.plot(
        df_dscovr.index.values, df_dscovr.flux.values, "g-", lw=lw, ms=ms, label=r"flux"
    )
    im4a = axs4.axhline(
    ...
    if df_dscovr.flux.isnull().all():
        ...
        axs4.set_ylim(
            np.nanmin([0.9 * np.nanmin(df_dscovr.flux), 2.4]),
    ...
    if (
        df_dscovr.lambda_phi.isnull().all()
        and df_dscovr.lambda_wav.isnull().all()
        and df_dscovr.lambda_vas.isnull().all()
        and df_dscovr.lambda_ekl.isnull().all()
    ):
        axs5.set_ylim([-1, 1])
    else:
    ...
    plt.close("all")
    print(
        "Figure saved for DSCOVR 54 days at (UTC):"
        + f"{datetime.datetime.utcfromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')}\n"
    )
